...
REPORT_COLUMNS = {
    ...
    PLAYER_PITCHING: ["ID", "IPClean", "WAA", "WAAAdj", "WAA200"],
    LEAGUE_BATTING: [
        "ID",
        "Unnamed: 0",
        "Team",
...
class ExcelReportWriter:
    ...
    def __init__(self, league: str):
        self.league = league
        self.filename = f"{REPORTS_PATH}/{league}Tables.xlsx"
    ...
    def write_potential_predictions30(self, df_predictions: pd.DataFrame):

        df_predictions = df_predictions[REPORT_COLUMNS[POTENTIALS30]]
        ...
        with pd.ExcelWriter(**args) as writer:
            df_predictions.to_excel(writer, sheet_name=POTENTIALS30, index=False)

        wb = openpyxl.load_workbook(filename=self.filename)
        if POTENTIALS30 not in wb[POTENTIALS30].tables:
            tab = openpyxl.worksheet.table.Table(
                displayName=POTENTIALS30,
                ref=f"A1:{openpyxl.utils.get_column_letter(df_predictions.shape[1])}{len(df_predictions)+1}",
            )
            wb[POTENTIALS30].add_table(tab)
            wb.save(self.filename)
